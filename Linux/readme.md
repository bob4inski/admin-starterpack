...
Что такое процесс - структура данных task_struct в которой хранятся все данные об адресном пространстве с которым мы взаимодействуем
...
5. Nginx что это такое
Это базовый http сервер который умеет отдавать какие-то странички, редиректить и делать грязь
...
### Задание
1. Попробовать поработать в NGINX
    - установить nginx - Если установил - плохо
    - поменять дефолтный порт который он слушает
    - поменять команду с помощью которой запускается nginx (посмотреть с какими аргументами можно запустить nginx и ExecStart)
    - поменять default страничку чтобы что-то другое было (А какая дефолт?)
    - Посмотреть в каком формате nginx по дефолту пишет свои логи
    - настроить кастомную запись логов через tail/journalctl (добавить несколько полей к записи лога)
    - Настроить nginx так, чтобы он логировал каждый запрос в /var/log/nginx/access.log
    ```bash
    ::1 - - [19/Oct/2025:15:38:16 +0000] "GET / HTTP/1.1" 200 615 "-" "curl/8.5.0"
    ::1 - - [19/Oct/2025:15:38:16 +0000] "GET / HTTP/1.1" 200 615 "-" "curl/8.5.0"
    ```
    * - Добавить URI/ручку nginx которая может отдавать ок/не ок


2. Информация про твою систему
    - освоить команды free, top, dh, atop, lsblk, df (тут можно посмотреть что дают на выходе все эти команды)
    - научиться шорткатам в VIM
    - Научиться использовать правильно консоль
    В консоли есть много сочетаний для перемещения по строке, к примеру:
...
3. Дебаг приложений
Есть такие улититы как strace, lsof и прочие, которые можно использовать для дебага приложений
Задача - в соседнем репозитории найти задачки и их решить https://github.com/bob4inski/sirius-tasks/blob/main/3.md
Спойлер - это бинарники, которе должны запуститься каким-то образом

* Надо еще найти полезные флаги strace для отладки
